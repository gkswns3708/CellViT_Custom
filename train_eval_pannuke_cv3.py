# -*- coding: utf-8 -*-
"""
PanNuke 3-fold CV 인스턴스 수준 평가 (논문 표와 동일한 방식)
...
또는 --ckpt_list 로 각 fold별 경로를 직접 지정할 수 있습니다.
"""
...
def iou_matrix(gt_map: np.ndarray, pr_map: np.ndarray) -> np.ndarray:
    ...
    # Build dict of pixel sets (or counts via histogram trick)
    iou = np.zeros((G, P), dtype=np.float32)
    # Faster via intersections computed with masks
    ...
    def add_image(
        ...
        for gi, pj, _ in matches:
            ...
            if pcls < 1 or pcls > self.C:
                # predicted as bg or invalid → just count prediction mass to that (if 0 ignore)
                continue
            # accumulate
            self.gt_c[gcls] += 1
            self.pred_c[pcls] += 1
...
def types_from_map_majority(type_argmax: np.ndarray, inst_map: np.ndarray) -> Dict[int, int]:
    ...
    for pid in ids:
        ...
        # ignore 0(bg), pick the most frequent >0; if all 0, keep 0 (ignored later)
        vals = vals[vals > 0]
        if vals.size == 0:
            cls = 0
        else:
...
def eval_one_fold(
    ...
    acc = InstanceMetricsAccumulator(num_classes_including_bg=model_cfg["num_classes"])
    ...
    for batch in dl:
        ...
        # pred_inst_maps_t: Tensor (B,H,W)
        pred_inst_maps = pred_inst_maps_t.cpu().numpy().astype(np.int32)

        # predicted per-instance class assignment (fallback to majority)
        type_argmax = torch.argmax(y["nuclei_type_map"], dim=1).cpu().numpy()  # (B,H,W)

        # NOTE: model.calculate_instance_map may already return type dicts.
        # If not usable, fallback to majority vote.
        for b in range(x.size(0)):
            ...
            # Pred dict: from postprocessor when available?
            pr_types: Dict[int, int] = {}
            if pred_type_dicts and isinstance(pred_type_dicts[b], dict) and len(pred_type_dicts[b]) > 0:
                # Keep only 1..5 range
                pr_types = {int(k): int(v) for k, v in pred_type_dicts[b].items()}
            else:
...
def parse_args():
    ...
    p.add_argument("--magnification", type=int, default=40, help="Detection postproc magnification (PanNuke=x40)")
    return p.parse_args()
